...
async function seedAll(prisma) {
    // роли
    for (const name of ['ADMIN', 'HEAD', 'EMPLOYEE']) {
    ...
    const pass = async (p) => bcrypt_1.default.hash(p, Number(process.env.PASSWORD_SALT_ROUNDS || 10));
    ...
    const emp_sales = await Promise.all([1, 2, 3].map(async (n) => prisma.user.upsert({
    ...
    const emp_ops = await Promise.all([1, 2, 3].map(async (n) => prisma.user.upsert({
    ...
    const emp_it = await Promise.all([1, 2, 3].map(async (n) => prisma.user.upsert({
    ...
    // direct managers
    for (const e of emp_sales)
        await prisma.directManager.upsert({ where: { employeeId: e.id }, update: { managerId: head_sales.id }, create: { employeeId: e.id, managerId: head_sales.id } });
    for (const e of emp_ops)
        await prisma.directManager.upsert({ where: { employeeId: e.id }, update: { managerId: head_ops.id }, create: { employeeId: e.id, managerId: head_ops.id } });
    for (const e of emp_it)
        await prisma.directManager.upsert({ where: { employeeId: e.id }, update: { managerId: head_it.id }, create: { employeeId: e.id, managerId: head_it.id } });
    ...
    for (const a of admins)
        await prisma.chatMember.create({ data: { chatId: leadership.id, userId: a.id } });
    for (const u of [...admins, ...heads])
        await prisma.chatMember.create({ data: { chatId: leadership_heads.id, userId: u.id } });
    for (const u of [head_sales, ...emp_sales])
        await prisma.chatMember.create({ data: { chatId: deptSales.id, userId: u.id } });
    for (const u of [head_ops, ...emp_ops])
        await prisma.chatMember.create({ data: { chatId: deptOps.id, userId: u.id } });
    for (const u of [head_it, ...emp_it])
        await prisma.chatMember.create({ data: { chatId: deptIt.id, userId: u.id } });
    // тестовые DM
