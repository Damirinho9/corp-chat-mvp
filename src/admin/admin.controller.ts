...
export class AdminController {
  ...
  async updateUser(
    @Req() req: any,
    @Body() body: { displayName?: string; role?: string; departmentId?: number | null; managerId?: number | null },
    ...
    if (body.hasOwnProperty("managerId")) data.managerId = body.managerId ?? null;
  ...
  private async syncUserSystemGroupsFull(
    ...
    await this.prisma.$transaction([
      ...toAdd.map(id => this.prisma.chatMember.create({ data: { chatId: id, userId: user.id } })),
      ...toRemove.map(id => this.prisma.chatMember.deleteMany({ where: { chatId: id, userId: user.id } })),
    ]);
  }
  ...
  @Post("users")
  async createUser(@Req() req: any, @Body() body: { username: string; displayName: string; role: string; departmentId?: number; managerId?: number; password: string }) {
    await this.assertAdmin(req);
    const pwd = body.password || '';
    const tooShort = pwd.length < 8;
    const sameAsUser = pwd.toLowerCase() == (body.username||'').toLowerCase();
    ...
    const user = await this.prisma.user.create({ data: { username: body.username, displayName: body.displayName, role: body.role as any, departmentId: body.departmentId || null, managerId: body.managerId || null, passwordHash: hash } });
  ...
  async listUsers(@Req() req: any) {
    await this.assertAdmin(req);
    return this.prisma.user.findMany({ orderBy: { id: 'asc' }, select: { id: true, username: true, displayName: true, role: true, departmentId: true, managerId: true } });
  }
}
